...
exports.getOrderById = async (parametres) => {
  try {
    const orderId = parseInt(parametres.req.params.id, 10);
...
exports.updateOrder = async (parametres) => {
  console.log('Connection:' + parametres.connection);
  console.log('Request:' + parametres.req);
  if (parametres.req.body != null) {
    const orderId = parseInt(parametres.req.params.id, 10);
    let body = parametres.req.body;
    try {
      const query =
      ...
      if (parametres.connection) {
        try {
          await parametres.connection.close();
        }
        catch (error) {
          console.error(error);
        }
...
exports.deleteOrder = async (parametres) => {
  const id = parseInt(parametres.req.params.id, 10);
  try {
    await parametres.connection.execute('DELETE FROM ORDER_ITEM WHERE ORDER_ID = :id', { id: id });
    await parametres.connection.execute(`DELETE FROM SERVICE_PRODUCT WHERE ID IN (SELECT SERVICE_PRODUCT_ID FROM ORDER_ITEM WHERE ORDER_ID = :id)`, { id: id });
    const result = await parametres.connection.execute('DELETE FROM USER_ORDER WHERE ID = :id', { id: id });
    await parametres.connection.execute(`DELETE FROM ORDER_STATUS WHERE ID IN (SELECT ORDER_STATUS_ID FROM USER_ORDER WHERE ID = :id)`, { id: id });
