...
exports.getProductByName = async (parametres) => {
    try {
        const name = parametres.name;
        const query = `SELECT * FROM PRODUCT WHERE PRODUCT_NAME = :NAME`;
        const binds = {NAME: name};
        const result = await parametres.connection.execute(query,binds);
        return result;
    } catch (error) {
        console.log(error);
    }
